...
본 프로젝트에서는 뉴스 및 소셜 데이터를 AI 기술로 분석하여 입법 수요를 선제적으로 탐지하고 정량화하는 End-to-End 자동화 서비스를 성공적으로 개발했습니다. RAG 아키텍처를 도입하여 복잡한 법률 조항을 자동 매핑하는 핵심 문제를 해결했으며, 4-Class 여론 분류 모델로 정책 방향성을 구체적으로 제시합니다. 또한, 6가지 다각적 지표(IIS)를 통해 이슈의 중요도를 객관적으로 산정하고, 모든 분석 결과를 웹 대시보드로 시각화하여 데이터 기반의 신속한 입법 및 정책 결정을 지원하는 기술적 토대를 마련했습니다.
...
- LegislativeStanceArea
    ...
    - useMEmo로 증감률 및 요약 인사이트를 계산하여 겐더링 성늠을 최적화
