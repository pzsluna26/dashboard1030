...
import { useSocialBar } from '../hooks/useSocialBar';
import { formatCategory } from '@/shared/constants/mapping';

function LegendSwatch({ color, label }: { color: string; label: string }) {
  return <span className="inline-flex items-center gap-1"><span className="inline-block w-3 h-3 rounded-sm" style={{ background: color }} /> <span>{label}</span></span>;
}
const COLORS = { reinforce: '#f59c9cff', repeal: '#9abdf7ff', oppose: '#9CA3AF' } as const;
const maxCount = (rows: any[]) => Math.ceil(rows.reduce((m, r) => Math.max(m, r.reinforce, r.repeal, r.oppose), 0));

export default function SocialBarChart({ startDate, endDate }: { startDate?: string; endDate?: string }) {
  const { data, loading, error } = useSocialBar(startDate, endDate);
  const [mode, setMode] = useState<'percent'|'count'>('percent');

  type Row = {
    category: string; reinforce: number; repeal: number; oppose: number; total: number;
    reinforcePct: number; repealPct: number; opposePct: number;
  };

  const chartData: Row[] = useMemo(() => {
    const safePct = (n: number, t: number) => t > 0 ? +(Math.round((n / t) * 1000) / 10).toFixed(1) : 0;
    return (data || []).map((r) => {
      ...
      return {
        category: r.category, reinforce, repeal, oppose, total,
        reinforcePct: safePct(reinforce, total), repealPct: safePct(repeal, total), opposePct: safePct(oppose, total),
      };
    });
  }, [data]);
  ...
  if (loading) return <div className="w-full h-[200px] grid place-items-center text-neutral-500">데이터 불러오는 중...</div>;
  if (error)   return <div className="w-full h-[200px] grid place-items-center text-rose-500 text-sm">오류 발생: {error}</div>;
  if (!chartData.length) return <div className="w-full h-[200px] grid place-items-center text-neutral-500 text-sm">데이터가 없습니다.</div>;

  return (
    <div className="w-full h-full flex flex-col">
      <div className="flex items-center justify-between mb-3 w-full">
        <div className="flex items-center gap-3 text-sm">
          <span className="px-2 py-1 rounded-full bg-white/70 border border-white/60">법안별 여론 성향</span>
          ...
          <div className="inline-flex rounded-xl overflow-hidden border border-neutral-200 bg-white/80">
            <button className={`px-3 py-1 text-sm ${mode==='percent'?'bg-neutral-900 text-white':'text-neutral-700 hover:bg-neutral-100'}`} onClick={() => setMode('percent')}>% 비율</button>
            <button className={`px-3 py-1 text-sm ${mode==='count'?'bg-neutral-900 text-white':'text-neutral-700 hover:bg-neutral-100'}`} onClick={() => setMode('count')}>건수</button>
          </div>
        </div>
      ...
      <div className="relative flex-1 w-full h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} barSize={22} margin={{ top: 8, right: 12, left: 0, bottom: 12 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
            <XAxis dataKey="category" tick={{ fontSize: 12, fill: '#4b5563' }} tickFormatter={formatCategory} />
            <YAxis tick={{ fontSize: 12, fill: '#4b5563' }} width={40}
              domain={mode==='percent'?[0,100]:[0,(maxCount(chartData)*1.1)||10]}
              tickFormatter={(v) => mode==='percent'?`${v}%`:`${Math.round(v)}`} />
    ...
    <div className="rounded-lg border border-neutral-200 bg-white/95 px-3 py-2 shadow-sm">
      ...
      {lines.map((l) => (
        <div key={l.key} className="flex items-center gap-2 text-xs">
          ...
          <span className="font-medium text-neutral-800">{mode === 'percent' ? `${l.v}%` : Number(l.v).toLocaleString()}</span>
        </div>
      ))}
    </div>
  );
